...
// Drawer open / close + Work gallery expand/collapse（合并到同一个 click，避免冲突）
document.addEventListener("click", (e) => {
  /* ===== 1) 展开/收起：首图下的“点击查看更多（n）” ===== */
  const moreBtn = e.target.closest(".work-more[data-target]");
  if (moreBtn) {
    e.preventDefault();

    const galleryId = moreBtn.getAttribute("data-target");
